...
@interface PTAppDelegate() <NSPortDelegate>
...
- (void)launchInputSourceThread
{
    ...
    if(myPort){
        ...
        thread.outPort = _port;
        [[NSRunLoop currentRunLoop] addPort:_port forMode:NSDefaultRunLoopMode];
    }
    
    [thread start];
}
#pragma mark:  注意：虽然有这么棒的方式实现线程间通讯方式，但是估计是由于危及iOS的Sandbox沙盒环境，所以这些API都是私有接口，如果你用到NSPortMessage，XCode会提示'NSPortMessage' for instance message is a forward declaration

#pragma mark: NSPortDelegate
- (void)handlePortMessage:(NSPortMessage *)portMessage{
    NSLog(@"======== NSPort message..");
    
    
    /*unsigned int message = [portMessage mgsId];
...
- (void)registerSource:(RunLoopContext*)sourceContext
{
    ...
    [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(fireSource) userInfo:nil repeats:YES];
    //[self fireSource];
}
